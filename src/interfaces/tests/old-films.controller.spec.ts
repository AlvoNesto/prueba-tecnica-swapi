...
describe('OldFilmsController', () => {
  ...
  describe('getSwapiFilmById', () => {
    it('should return a single film by ID', async () => {
      ...
      jest.spyOn(filmsService, 'getSwapiFilmById').mockResolvedValue(mockFilm);

      const result = await oldFilmsController.getSwapiFilmById(filmId);

      expect(result).toEqual(mockFilm);
      expect(filmsService.getSwapiFilmById).toHaveBeenCalledWith(filmId);
      expect(filmsService.getSwapiFilmById).toHaveBeenCalledTimes(1);
    });
  });
});
